package com.zzx.loginfail_detect.udf_source;
 ...
 * @ClassName: LoginEventSource
 * @author: wb-zcx696752
 * @description:
 * @data: 2021/4/12 17:09 PM
 */
public class LoginEventSource implements SourceFunction<LoginEvent> {
    ...
    public void run(SourceContext<LoginEvent> ctx) throws Exception {
        ...
        List<String> states = Arrays.asList("success", "fail");

        while (isRunning) {
            ctx.collect(new LoginEvent(
                    Long.valueOf(new Random().nextInt(2)),
                    getRandomIp(),
//                    states.get(new Random().nextInt(states.size())),
                    "fail",
